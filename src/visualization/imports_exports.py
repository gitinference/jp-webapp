...
def web_app_imports_exports(request):
    ...
    if request.method == "POST":
        ...
        imports_graph = requests.get(
            f"{api}graph/import-export/?level=country&time_frame=yearly&group=false&frequency=Yearly&second_dropdown=2025&datetime=2025&type=imports"
        )
    ...
    if request.method == "POST":
        ...
        exports_graph = requests.get(
            f"{api}graph/import-export/?level=country&time_frame=yearly&group=false&frequency=Yearly&second_dropdown=2025&datetime=2025&type=exports"
        )
